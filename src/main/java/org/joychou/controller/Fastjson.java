package org.joychou.controller;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.alibaba.fastjson.parser.Feature;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.ResponseBody;


@Controller
@RequestMapping("/fastjson")
public class Fastjson {

    /**
     * Deserializes the provided JSON string to extract the "name" property.
     *
     * <p>
     * This method converts the input JSON string into a JSON object and attempts to retrieve the
     * value associated with the "name" key. If the extraction is successful, it returns the string
     * representation of that value. If any exception occurs during parsing or extraction, the method
     * returns the exception's string representation.
     * </p>
     *
     * @param params the JSON string received in the request body
     * @return the value of the "name" property if extraction is successful; otherwise, the exception message
     */
    @RequestMapping(value = "/deserialize", method = {RequestMethod.POST})
    @ResponseBody
    public String Deserialize(@RequestBody String params) {
        // 如果Content-Type不设置application/json格式，post数据会被url编码
        try {
            // 将post提交的string转换为json
            JSONObject ob = JSON.parseObject(params);
            return ob.get("name").toString();
        } catch (Exception e) {
            return e.toString();
        }
    }

    /**
     * Demonstrates deserialization of a hardcoded JSON payload using Fastjson with support for non-public fields.
     *
     * <p>This method defines a JSON payload that encapsulates a serialized object containing embedded bytecode,
     * potentially crafted to trigger a security test (as hinted by the comment "Open calc in mac"). It then invokes
     * {@code JSON.parseObject} with the {@link Feature#SupportNonPublicField} feature enabled. This example is provided
     * for demonstration purposes only.
     *
     * @param args command line arguments (not used)
     */
    public static void main(String[] args) {
        // Open calc in mac
        String payload = "{\"@type\":\"com.sun.org.apache.xalan.internal.xsltc.trax.TemplatesImpl\", \"_bytecodes\": [\"\"], \"_name\": \"lightless\", \"_tfactory\": { }, \"_outputProperties\":{ }}";
        JSON.parseObject(payload, Feature.SupportNonPublicField);
    }
}
